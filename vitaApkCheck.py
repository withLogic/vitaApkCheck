...
apks_in_spreadsheet = worksheet.col_values(1)
apks_in_spreadsheet = list(filter(None, apks_in_spreadsheet))
...
for row in rows:
	...
	if start_row_value != current_row_value and start_match == 1:
		...
		worksheet.merge_cells(f"B{start_row}:B{end_row}","MERGE_COLUMNS")
		worksheet.merge_cells(f"C{start_row}:C{end_row}","MERGE_COLUMNS")
		worksheet.merge_cells(f"D{start_row}:D{end_row}","MERGE_COLUMNS")
		worksheet.merge_cells(f"E{start_row}:E{end_row}","MERGE_COLUMNS")
		worksheet.merge_cells(f"F{start_row}:F{end_row}","MERGE_COLUMNS")
		worksheet.merge_cells(f"G{start_row}:G{end_row}","MERGE_COLUMNS")
		worksheet.merge_cells(f"H{start_row}:H{end_row}","MERGE_COLUMNS")
		worksheet.merge_cells(f"I{start_row}:I{end_row}","MERGE_COLUMNS")

		# sleeping to avoid being rate limited by Google
		time.sleep(40)
